...
```{r}
df <- read_csv("data/AdquisicionAhorro.csv")
kable(df[1:12, ])
```
...
## Organización de datos y visualización de gráficos
...
Primero, graficamos `balance`.

```{r}
full_summary(df$balance)
hist(df$balance, main = "Histograma del balance de los encuestados")
```
...
```{r}
full_summary(df$educacion)

educacion_table <- table(df$educacion)
bars <- barplot(educacion_table, col = rainbow(3), ylim = c(0, 1500), main = "Educación de los encuestados")
text(bars, educacion_table + 100, labels = educacion_table)
```
...
Sin embargo, resulta que este subconjunto que vamos a graficar sólo tiene `r unique(filter_outliers(df$campana))` valores únicos, así que parece mejor usar un gráfico de barras.
...
En términos de frecuencias relativas, podemos observar mejor a `campana` con un gráfico circular:
