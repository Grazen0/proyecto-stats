...
Para este proyecto, la columna `coddoc` no es de utilidad, porque no provee algún dato estadístico relevante sobre las observaciones. Tal parece que sólo sirve como identificador de las observaciones.

```{r}
df <- df %>% select(-coddoc)
```

### Arreglo de los tipos de variables
...
Por otro lado, una observación a la columna `duracion` muestra que existe un dato que tiene valor decimal:

```{r}
sum(!is.na(df$duracion) & (as.integer(df$duracion) != df$duracion))
```
...
```{r}
summary(df %>% select(edad, balance, dia, duracion, campana, pdias, previo))
```
...
```{r}
# is.na, pero no retorna TRUE para NANs
is.na.strict <- function(x) {
  is.na(x) & !sapply(x, is.nan)
}

# Esta función calcula los valores faltantes de cada columna
# sin contar los `NaN`.
na_check <- function(x) {
  apply(is.na.strict(x), 2, sum)
}
...
cvs <- apply(df %>% select(edad, balance, dia, duracion, campana, previo), 2, cv)
cvs
```
...
```{r}
df <- df %>% filter(!is.na.strict(df$pdias))
```
...
```{r}
df <- df %>% filter(!is.na.strict(df$adq_ahorro))
```
...
```{r}
apply(df %>% select(estciv, educacion, mora, vivienda, prestamo, adq_ahorro), 2, unique)
```

De esta manera, podemos comprobar que las variables categóricas tienen valores consistentes.

:::{.callout-note}
Nótese que contamos a `adq_ahorro` como categórico porque es esencialmente un sí/no codificado en 1/0.
:::
