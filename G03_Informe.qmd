...
En otras palabras, existe un `r 100 * r(ans)`% de probabilidad de seleccionar exactamente 1500 clientes que hayan sido contactados no más de 2 veces durante la presente campaña.
...
$$
P(1300 \leq X_1 \leq 1500) = P(X_1 \leq 1500) - P(X_1 \leq 1299) = `r r(ans)`
.$$

En otras palabras, existe un `r 100 * r(ans)`% de probabilidad de seleccionar entre 1300 y 1500 clientes que hayan sido contactados no más de 2 veces durante la presente campaña. Tiene sentido que esta probabilidad sea tan alta, puesto que el rango entre $1300$ y $1500$ contiene a la media.
...
- $P(X_2 = 0)$: Probabilidad de que ningún cliente del subconjunto de $`r n`$ tenga al menos 60 años.
- $P(X_2 = 1)$: Probabilidad de que exactamente 1 cliente del subconjunto de $`r n`$ tenga al menos 60 años.
- ...
- $P(X_2 = `r n`)$: Probabilidad de que todos los clientes del subconjunto de $`r n`$ tengan al menos 60 años.
...
En otras palabras, existe un `r 100 * r(ans)`% de probabilidad de que, de un subconjunto aleatorio de $1000$ de entre el total de $`r nrow(df_adq)`$ que adquirieron la cuenta, exactamente $45$ personas tengan 60 años o más.
...
donde $p = `r r(p)`$, la probabilidad de éxito, es igual a la probabilidad empírica de seleccionar al azar un cliente no contactado previamente (es decir, con `pdias == -1`) que no haya adquirido la cuenta de ahorro.
...
Esto es, existe un `r 100 * r(ans)`% de probabilidad de que tome exactamente 10 selecciones de clientes al azar (nuevamente, de entre quienes no hayan sido contactados) para finalmente hallar uno que no adquiera la cuenta de ahorro.
...
lines(x, dexp(x, 1 / mean(df_adq$balance)), col = "red", lwd = 2)
```

Aunque la @fig-distribution-x4 muestra una apariencia posiblemente exponencial, se puede comprobar numéricamente que la condición $\operatorname{E}(X_4) = \operatorname{SD}(X_4)$, de una distribución exponencial, *no se cumple* por un gran margen:
...
lambda <- 1 / mean(df_adq_filt$balance)
...
Cabe destacar que este cambio, de todas formas, preserva el `r r(100 * length(df_adq_filt$balance) / length(df_adq_filt$balance))`% de los balances originales.
...
Siguen sin ser exactamente iguales, pero ahora el error relativo es de `r r(100 * abs(mean(df_adq_filt$balance) - sd(df_adq_filt$balance)) / sd(df_adq_filt$balance))`% y la curva exponencial encaja algo mejor en la gráfica. El ajuste sigue sin estar muy cerca de ser exacto, pero está lo suficientemente cerca como para usarlo en cálculos que no requieran mayor precisión.
...
donde $\beta$, la tasa, es igual a la media empírica de los balances menores a $`r max_bal`$ euros.
...
$$
P(X_4 \ge 2000) = `r r(pexp(2000, lambda, lower.tail = FALSE))`
.$$
...
1. Los balances de quienes adquirieron la cuenta de ahorro son, en general, superiores a los de quienes no la adquirieron, y una proporción considerable se encuentra entre 500 y 1000 euros. Además, se observa una proporción ligeramente mayor de personas con historial de mora que aceptan la cuenta de ahorro en comparación con aquellas sin mora.
2. La mayoría de los encuestados con edades atípicas (mayores de 70 años) adquirieron la cuenta de ahorro. Además, existe una probabilidad de 0.34 de que entre los clientes que aceptan la cuenta el 4.5% tengan al menos 60 años. No obstante, en general, las distribuciones de edad entre ambos grupos presentan medias y medianas similares. Asimismo, entre quienes adquirieron la cuenta se observa una proporción ligeramente menor de individuos casados.
...
4. El número promedio de contactos no varía significativamente entre quienes adquieren la cuenta y quienes no. Sin embargo, en algunos casos, un mayor número de contactos parece estar asociado con una mayor probabilidad de adquisición. Además, la mayoría de los encuestados tomaron la decisión de aceptar o rechazar la cuenta en el primer contacto, lo que sugiere la importancia de la primera impresión del banco en la decisión del cliente. Por otro lado, la probabilidad de que un cliente no contactado adquiera la cuenta es relativamente alta; sin embargo, la ocurrencia consecutiva de más de 10 clientes no contactados que sí la adquieran es poco probable.
