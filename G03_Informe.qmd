...
> $X_3$: Balance de una persona seleccionada al azar de la muestra de $`r nrow(df)`$ del estudio.

De esta manera, definimos la siguiente probabilidad:

> $P(a \leq X_3 \leq b)$: Probabilidad de que el balance de una persona seleccionada al azar del estudio se encuentre entre $a$ y $b$ euros.

:::{.callout-note}
Quiero preguntarle al profe si se puede hacer eso de ignorar algunos datos atipicos para q salga mejor el modelo.
:::

Vamos a mostrar que $X_3$, en general, sigue el modelo de distribución **exponencial**. A continuación se muestra la distribución empírica de $X_3$ en un radio de $3\operatorname{SD}(X_3)$ alrededor de $\operatorname{E}(X_3)$, junto a una curva de distribución exponencial generada con $\beta = \operatorname{E}(X_3)$:
...
Supongamos que $X_3$ sigue la distribución

$$
X_3 \sim \operatorname{Exp}(\beta)
,$$

donde $\beta = \operatorname{E}(X_3) = `r r(b_mean)`$ es la media empírica de $X_3$.
...
$$
P(X_3 \ge 2000) = `r r(pexp(2000, 1 / b_mean, lower.tail = FALSE))`
.$$
